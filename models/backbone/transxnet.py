...
def transxnet_t(pretrained=False, pretrained_cfg=None, **kwargs):
    ...
    return model
def transxnet_xs(pretrained=False, pretrained_cfg=None, **kwargs):
    model = TransXNet(arch='xs', **kwargs)
    model.default_cfg = _cfg(crop_pct=0.9)
    if pretrained:
...
def transxnet_4xs(pretrained=False, pretrained_cfg=None, **kwargs):
    model = TransXNet(arch='4xs', **kwargs)
    model.default_cfg = _cfg(crop_pct=0.9)
    if pretrained:
        ckpt = 'https://github.com/LMMMEng/TransXNet/releases/download/v1.0/transx-s.pth.tar'
        load_checkpoint(model, ckpt)
    return model
def transxnet_5xs(pretrained=False, pretrained_cfg=None, **kwargs):
    model = TransXNet(arch='5xs', **kwargs)
    model.default_cfg = _cfg(crop_pct=0.9)
    if pretrained:
        ckpt = 'https://github.com/LMMMEng/TransXNet/releases/download/v1.0/transx-s.pth.tar'
        load_checkpoint(model, ckpt)
    return model
def transxnet_3xs(pretrained=False, pretrained_cfg=None, **kwargs):
    model = TransXNet(arch='3xs', **kwargs)
    model.default_cfg = _cfg(crop_pct=0.9)
    if pretrained:
        ckpt = 'https://github.com/LMMMEng/TransXNet/releases/download/v1.0/transx-s.pth.tar'
        load_checkpoint(model, ckpt)
    return model
def transxnet_xxs(pretrained=False, pretrained_cfg=None, **kwargs):
    model = TransXNet(arch='xxs', **kwargs)
    model.default_cfg = _cfg(crop_pct=0.9)
    if pretrained:
        ckpt = 'https://github.com/LMMMEng/TransXNet/releases/download/v1.0/transx-s.pth.tar'
        load_checkpoint(model, ckpt)
